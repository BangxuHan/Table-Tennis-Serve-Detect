...
save_folder = 'saved/TSSTG(pts+mot)-04(migu10)'
# data_files = ['/home/kls/data/tabletennisdata/xigua_set(labelXscrw).pkl',
#               '/home/kls/data/tabletennisdata/kls_set(labelXscrw25).pkl']
data_files = ['/mnt/marathon/pingpangclips/migu2022/migu_set(labelXscrwv4).pkl']
...
if __name__ == '__main__':
    ...
    train_loader = data.DataLoader(train_loader.dataset, batch_size, shuffle=True)
    dataloader = {'train': train_loader, 'valid': valid_loader}
    ...
    for e in range(epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            with tqdm(dataloader[phase], desc=phase) as iterator:
                for pts, lbs in iterator:
                    # Create motion input by distance of points (x, y) of the same node in two frames
                    mot = pts[:, :2, 1:, :] - pts[:, :2, :-1, :]
        ...
        plot_graphs(list(loss_list.values()), list(loss_list.keys()),
                    ...
                    'Loss', xlim=[0, epochs], save=os.path.join(save_folder, 'loss_graph.png'))
        plot_graphs(list(accu_list.values()), list(accu_list.keys()),
                    'Last Train: {:.2f}, Valid: {:.2f}'.format(accu_list['train'][-1], accu_list['valid'][-1]),
                    'Accu', xlim=[0, epochs], save=os.path.join(save_folder, 'accu_graph.png'))
    # break
    ...
    # EVALUATION
    model = set_training(model, False)
    # data_file = data_files[1]  # --- multiple dataset
    data_file = data_files[0]  # --- single dataset
    ...
    plot_confusion_metrix(y_trues, y_preds, class_names,
                          'Eval on: {}\nLoss: {:.4f}, Accu{:.4f}'.format(os.path.basename(data_file), run_loss,
                                                                         run_accu), 'true',
                          save=os.path.join(save_folder, '{}-confusion_matrix.png'.format(
